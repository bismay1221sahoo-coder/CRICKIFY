...
test("home page loads with hero and nav", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("banner").getByRole("link", { name: /CRICKIFY/i })).toBeVisible();
...
test("listing details route handles missing listing gracefully", async ({ page }) => {
  ...
  await expect(page.getByText(/listing not found|not found|request failed|failed/i)).toBeVisible();
});
